...
const isOdooImage = (item: GridImageItem): boolean => {
  ...
  if (
    ...
    (item.base64Data && !item.base64Data.startsWith('/'))
  ) {
    return true;
  }
...
export function PCFImagesPage({ assortment }: PCFImagesPageProps) {
  ...
  const [innerCartonShippingMarks, setInnerCartonShippingMarks] = useState<
    GridImageItem[]
  >([]);
  const [masterCartonMainShippingMarks, setMasterCartonMainShippingMarks] =
    useState<GridImageItem[]>([]);
  const [masterCartonSideShippingMarks, setMasterCartonSideShippingMarks] =
    useState<GridImageItem[]>([]);
  ...
  const assortmentItemNo =
    assortment?.baseAssortment?.itemNo || (assortment as any)?.itemNo || '';
  const { data: freshMongoData } = useGetWebhookAssortment(assortmentItemNo, {
    enabled: !!assortmentItemNo,
    staleTime: 0,
    gcTime: 0,
    refetchOnMount: true,
  ...
  const assortmentId =
    actualAssortment?.baseAssortment?.itemNo ||
  ...
  const hasUnsavedChanges = useMemo(() => {
    ...
    return (
      pendingDeletions.length > 0 || pendingReplacements.size > 0 || hasNewFiles
    );
  }, [
    pendingDeletions,
    pendingReplacements,
  ...
  const handleStageForDeletion = useCallback(
    (imageFilename: string) => {
      if (!pendingDeletions.includes(imageFilename)) {
        setPendingDeletions((prev) => [...prev, imageFilename]);
        toast.info(
          `Image marked for deletion. Click 'Save Changes' to confirm.`,
        );
      }
    },
    [pendingDeletions],
  ...
  const handleStageForReplacement = useCallback(
    (imageFilename: string, newFile: File, category?: string) => {
      ...
      if (!detectedCategory) {
        ...
        const allSections = [
          ...
          { items: innerCartonShippingMarks, category: 'innerCartonImages' },
          { items: masterCartonItems, category: 'masterCartonImages' },
          {
            items: masterCartonMainShippingMarks,
            category: 'masterCartonImages',
          },
          {
            items: masterCartonSideShippingMarks,
            category: 'masterCartonImages',
          },
        ...
        for (const section of allSections) {
          const foundImage = section.items.find(
            (item) =>
          ...
          if (foundImage) {
            ...
            console.log(
              `🔍 Detected category for ${imageFilename}: ${detectedCategory}`,
            );
            break;
          }
        }
      ...
      setInnerCartonShippingMarks(updateItemsWithReplacement);
      setMasterCartonMainShippingMarks(updateItemsWithReplacement);
      setMasterCartonSideShippingMarks(updateItemsWithReplacement);
      ...
      toast.info(
        `Image replacement staged for ${detectedCategory || 'unknown category'}. Click 'Save Changes' to confirm.`,
      );
    },
    [
      displayItems,
  ...
  const convertOdooImageToGridItem = useCallback(
    (odooImage: any, index: number): GridImageItem => {
  ...
  const convertUploadedFileToGridItem = useCallback(
    (uploadedFile: any, index: number, prefix: string): GridImageItem => {
  ...
  const initializeItemPackSectionsWithData = useCallback(
    (webhookImages: any, userMods: any): ItemPackSection[] => {
      const itemPackImages = webhookImages?.itemPackImages || [];
      const userUploadedItemPack =
        (userMods?.uploadedImages as any)?.itemPackImages || [];
      ...
      if (
        Array.isArray(userUploadedItemPack) &&
        userUploadedItemPack.length > 0
      ) {
        const userItems: GridImageItem[] = userUploadedItemPack
          .map((file: any, i: number) =>
            convertUploadedFileToGridItem(file, i, 'user-itempack'),
          )
          .filter((item) => !item.isMarkedForDeletion);
    ...
    (
      ...
      if (Array.isArray(userUploads)) {
        items.push(
          ...userUploads
            .map((file, i) => {
              return convertUploadedFileToGridItem(
                file,
                i,
                `user-${sectionName}`,
              );
      ...
      if (Array.isArray(shippingMarks)) {
        shippingMarks.forEach((file, i) => {
          ...
          const shippingMarkItem = convertUploadedFileToGridItem(
            file,
            i,
            `shipping-${sectionName}`,
          );
  ...
  const copyLastImages = useCallback(
    (
      ...
      itemPackSections.forEach((section, sectionIndex) => {
        const actualImages = section.items.filter(
          (item) => !item.isUploadSlot && !item.isMarkedForDeletion,
        );
      ...
      const actualDisplayImages = displayItems.filter(
        (item) => !item.isUploadSlot && !item.isMarkedForDeletion,
      );
      if (actualDisplayImages.length > 0) {
        ...
        const actualInnerImages = newInnerCartonItems.filter(
          (item) => !item.isUploadSlot && !item.isMarkedForDeletion,
        );
  ...
  const initializeAllSections = useCallback(
    (webhookImages: any, userMods: any) => {
      // First, initialize base sections without copying
      const baseItemPackSections = initializeItemPackSectionsWithData(
        webhookImages,
        userMods,
      );
      ...
      console.log('🔍 Debug userMods structure:', {
        ...
        innerCartonShippingMarks:
          userUploadedImages?.innerCartonShippingMarks?.length || 0,
        masterCartonMainShippingMarks:
          userUploadedImages?.masterCartonMainShippingMarks?.length || 0,
        masterCartonSideShippingMarks:
          userUploadedImages?.masterCartonSideShippingMarks?.length || 0,
      ...
      const baseInnerCartonItems = initializeImageSection(
        ...
        Array.isArray((userUploadedImages as any)?.innerCartonShippingMarks)
          ? (userUploadedImages as any).innerCartonShippingMarks
          : [], // ✅ NEW: Include shipping marks
        'inner-carton',
      ...
      const baseMasterCartonItems = initializeImageSection(
        ...
        [
          ...(Array.isArray(
            (userUploadedImages as any)?.masterCartonMainShippingMarks,
          )
            ? (userUploadedImages as any).masterCartonMainShippingMarks
            : []),
          ...(Array.isArray(
            (userUploadedImages as any)?.masterCartonSideShippingMarks,
          )
            ? (userUploadedImages as any).masterCartonSideShippingMarks
            : []),
      ...
      const { newDisplayItems, newInnerCartonItems, newMasterCartonItems } =
        copyLastImages(
      ...
      const hasInnerShippingMark =
        !!userUploadedImages?.innerCartonShippingMarks;
      const hasMainShippingMark =
        !!userUploadedImages?.masterCartonMainShippingMarks;
      const hasSideShippingMark =
        !!userUploadedImages?.masterCartonSideShippingMarks;
    ...
    [
      copyLastImages,
      initializeItemPackSectionsWithData,
  ...
  useEffect(() => {
    if (actualAssortment && assortmentId) {
      const currentWebhookImages =
        actualAssortment?.baseAssortment?.webhookImages ||
        (actualAssortment as any)?.pcfImages;
      const currentUserMods =
        freshMongoData?.userModifications ||
        actualAssortment?.userModifications ||
        null;
      ...
      console.log('🔍 DEBUG: Raw API response structure:', {
        ...
        directShippingMarkAccess:
          currentUserMods?.uploadedImages?.innerCartonShippingMarks,
      });
      ...
      console.log('🔍 DEBUG: Initializing sections with data:', {
        ...
        uploadedImagesKeys: currentUserMods?.uploadedImages
          ? Object.keys(currentUserMods.uploadedImages)
          : [],
        uploadedImagesRaw: currentUserMods?.uploadedImages, // ✅ NEW: See the raw object
        innerCartonShippingMarksRaw: (currentUserMods?.uploadedImages as any)
          ?.innerCartonShippingMarks, // ✅ NEW: Direct access
        innerCartonShippingMarks: Array.isArray(
          (currentUserMods?.uploadedImages as any)?.innerCartonShippingMarks,
        )
          ? (currentUserMods.uploadedImages as any).innerCartonShippingMarks
              .length
          : 0,
        masterCartonMainShippingMarks: Array.isArray(
          (currentUserMods?.uploadedImages as any)
            ?.masterCartonMainShippingMarks,
        )
          ? (currentUserMods.uploadedImages as any)
              .masterCartonMainShippingMarks.length
          : 0,
        masterCartonSideShippingMarks: Array.isArray(
          (currentUserMods?.uploadedImages as any)
            ?.masterCartonSideShippingMarks,
        )
          ? (currentUserMods.uploadedImages as any)
              .masterCartonSideShippingMarks.length
          : 0,
      });
      ...
      const {
        ...
        innerCartonItems,
        masterCartonItems,
      } = initializeAllSections(currentWebhookImages, currentUserMods);
    ...
    try {
      ...
      if (pendingReplacements.size > 0) {
        ...
        for (const [oldFilename, replacementData] of pendingReplacements) {
          ...
          const replaceOperation = deleteMutation
            .mutateAsync({
              assortmentId,
      ...
      const collectFilesFromSection = (
        items: GridImageItem[],
        category: string,
      ) => {
        items
          .filter((i) => i.file && i.isUploadSlot)
          .forEach((item) => {
            if (item.file) {
              ...
              if (
                newFiles[regularCategory] &&
                Array.isArray(newFiles[regularCategory])
              ) {
                (newFiles[regularCategory] as File[]).push(item.file);
                console.log(
                  `📤 Adding regular image: ${item.file.name} to ${category}`,
                );
              }
            }
          });
      ...
      collectFilesFromSection(innerCartonShippingMarks, 'innerCartonImages');
      collectFilesFromSection(masterCartonItems, 'masterCartonImages');
      collectFilesFromSection(
        masterCartonMainShippingMarks,
        'masterCartonImages',
      );
      collectFilesFromSection(
      ...
      const hasNewUploads = Object.values(newFiles).some(
        (arr) => arr.length > 0,
      );
      if (hasNewUploads) {
        const totalFiles = Object.values(newFiles).flat().length;
        const shippingMarkCount =
          newFiles.innerCartonShippingMarks.length +
          newFiles.masterCartonMainShippingMarks.length +
          newFiles.masterCartonSideShippingMarks.length;

        console.log(
          `📤 Uploading ${totalFiles} files with ${shippingMarkCount} shipping marks to dedicated fields:`,
        );
      ...
      if (failed.length > 0) {
        ...
        toast.error(
          `${failed.length} operations failed. Check console for details.`,
        );
      } else {
      ...
      setInnerCartonShippingMarks(clearReplacementFiles);
      setMasterCartonMainShippingMarks(clearReplacementFiles);
      setMasterCartonSideShippingMarks(clearReplacementFiles);
        ...
        <div className="text-sm text-gray-600">
          {hasUnsavedChanges && (
            <span className="text-orange-600 font-medium">
              Unsaved changes: {pendingDeletions.length} deletions,{' '}
              {pendingReplacements.size} replacements
            </span>
          )}
        </div>
        <Button
      ...
      <div className="bg-white rounded-lg shadow-sm px-4 py-1 mt-4">
            ...
            {itemPackSections.map((section) => (
              ...
              >
                ...
                <GridImageSection
                  ...
                  onStageForReplace={(filename, file) =>
                    handleStageForReplacement(filename, file, 'itemPackImages')
                  }
        ...
        >
          <GridImageSection
            ...
            onStageForDelete={handleStageForDeletion}
            onStageForReplace={(filename, file) =>
              handleStageForReplacement(filename, file, 'itemBarcodeImages')
            }
        ...
        >
          <GridImageSection
            ...
            onStageForDelete={handleStageForDeletion}
            onStageForReplace={(filename, file) =>
              handleStageForReplacement(filename, file, 'displayImages')
            }
        ...
        >
          <GridImageSection
            ...
            onStageForDelete={handleStageForDeletion}
            onStageForReplace={(filename, file) =>
              handleStageForReplacement(filename, file, 'innerCartonImages')
            }
        ...
        >
          <GridImageSection
            ...
            onStageForDelete={handleStageForDeletion}
            onStageForReplace={(filename, file) =>
              handleStageForReplacement(filename, file, 'masterCartonImages')
            }
